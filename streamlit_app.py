...
REVERIFY_INTERVAL = 300   # re-verify every 5 minutes in exam mode
...
with vc:
    ...
    capph = st.empty()
    mfph = st.empty()
    rvph = st.empty()   # re-verify banner
    grph = st.empty()   # graph
    pdfph = st.empty()   # pdf download
...
def _rcard(reason):
    ...
    return (
        '<div class="reason">'
        '<div class="reason-label">Spoof type detected</div>'
...
def _ccard(cr):
    ...
    return (
        '<div class="cb"><div class="cs">Step ' + str(done + 1) +
        ' of ' + str(tot) + '</div>'
...
def _reverify_banner(seconds_left):
    return (f'<div class="reverify">&#8635; Re-verification required in '
            f'{seconds_left:.0f}s</div>')
...
def render(sr, cr, blr, hdr, txr, mor, pur, tamper_result=None):
    ...
    if tamper_result and tamper_result.get("current_warning"):
        tph.markdown(
            _warn_banner(tamper_result["current_warning"]),
            unsafe_allow_html=True)
    else:
        tph.empty()
    ...
    blph.markdown(_sc("BLINKS", str(blr.get("blink_count", 0)),
                      f"EAR {blr.get('ear', 0):.3f}", "cyan"),
                  unsafe_allow_html=True)
    hdph.markdown(_sc("HEAD", hdr.get("direction", "---"),
                      f"Yaw {hdr.get('yaw', 0):+.1f}deg", "yellow"),
                  unsafe_allow_html=True)
    txph.markdown(_gc("TEXTURE", txr.get("texture_score", 0),
                      "teal", "#64ffda"),           unsafe_allow_html=True)
    moph.markdown(_gc("MOTION",  mor.get("score", 0),
                      "cyan", "#00e5ff"),            unsafe_allow_html=True)
    ...
    if coherence_val > 0.82:
        warnph.markdown(
            _warn_banner("Rigid motion detected — phone or photo suspected"),
            unsafe_allow_html=True)
    elif glare_s < 0.35:
        warnph.markdown(
            _warn_banner("Screen glare detected — phone spoof suspected"),
            unsafe_allow_html=True)
    else:
        warnph.empty()
...
def _finish_session(D, verdict):
    """Save log, generate graph, generate PDF, store paths in session state."""
    ...
    if REPORTLAB_AVAILABLE:
        try:
            ...
            pdf_path = generate_report(
                ...
                candidate_id=st.session_state.candidate_id or "N/A",
...
if st.session_state.running:
    ...
    if not cap.isOpened():
        st.error(
            f"Cannot access Camera {cam_index}. "
            "Try selecting a different camera or click Refresh Camera List.")
        st.session_state.running = False
    else:
        ...
        while st.session_state.running:
            ...
            if faces:
                ...
                st.session_state.last = (sr, cr, blr, hdr, txr, mor, pur,
                                         tamper_result)
    ...
    # render supports optional tamper_result as 8th element
    if len(last) == 8:
        render(*last)
    else:
        render(*last, None)
    vidph.info("Session complete. Press START / NEW for another check.")

    # Exam mode re-verify prompt
